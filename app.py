...
# Load textures from root and Textures/
textures = {}
materials = []
root = os.getcwd()
...
# Sidebar: load/save session, rotate, delete
st.sidebar.header("Configuration")
wall_width = st.sidebar.number_input("Wall width (cm)", value=400.0)
sf = st.sidebar.file_uploader("Load session", type=['json'])
if sf:
    data = json.load(sf)
...
for p in st.session_state.panels:
    ...
    divs.append(f"<div class='panel' id='{p['id']}' data-img='data:image/jpeg;base64,{img_data}' "
...
document.getElementById('exportBtn').onclick = () => {{
  ...
  base.onload=()=>{{
    ...
    document.querySelectorAll('.panel').forEach(panel=>{{
      ...
      img2.onload=()=>{{
        const pw=panel.offsetWidth*sc;const ph=panel.offsetHeight*sc;
        const px=parseFloat(panel.style.left)*sc;const py=parseFloat(panel.style.top)*sc;
...
# Save/share session
st.download_button("Save session", json.dumps({'panels':st.session_state.panels}), file_name='session.json')
