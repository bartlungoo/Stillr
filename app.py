...
# Zet page config als eerste
st.set_page_config(layout="wide")

# Toon het logo indien aanwezig
try:
...
# Textures inladen uit projectroot en Textures/
textures = {}
materials = []
root = os.getcwd()
...
# Sidebar-configuratie: sessies laden/opslaan, roteren, verwijderen
...
# Voorverwerking beeld & schaal
ing_64 = base64.b64encode(photo_bytes).decode()
...
with st.form("add_panel_form"):
    ...
    if st.form_submit_button("Add panel"):
        st.session_state.panels.append({
            ...
            'rotation': 0, 'size': psize, 'mat': mat
        })

# Bouw HTML-divs en init-scripts
divs, scripts = [], []
for p in st.session_state.panels:
...
<div id='wall'>
  <img src='data:image/jpeg;base64,{ing_64}' style='width:800px;' />
  {''.join(divs)}
</div>
<script>
...
# Na render: sync drag-posities via streamlit-js-eval
try:
    from streamlit_js_eval import streamlit_js_eval
    pos_list = streamlit_js_eval(js_expressions=[
        "(function(){var arr=[];document.querySelectorAll('.panel').forEach(function(el){var m=el.style.transform.match(/rotate\\(([-0-9.]+)deg\\)/);arr.push({id:el.id,x:parseInt(el.style.left),y:parseInt(el.style.top),rotation:m?parseFloat(m[1]):0});});return JSON.stringify(arr);})()"
    ], key='sync')
    if pos_list:
        pos_json = pos_list[0] if isinstance(pos_list, list) else pos_list
        arr = json.loads(pos_json)
        for pos in arr:
            for p in st.session_state.panels:
                if p['id']==pos['id']:
