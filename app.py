...
# Verberg extra padding
st.markdown("<style>.block-container {padding-top:1rem;}</style>", unsafe_allow_html=True)

# Paneel-formaten (breedte x hoogte in cm)
sizes = {"M": (47.5,95), "L": (95,95), "XL": (190,95), "MOON": (95,95)}

# Textures inladen uit root & Textures/
textures, materials = {}, []
root = os.getcwd()
...
# Basis64 en schaal voor UI
photo_b64 = base64.b64encode(photo_bytes).decode()
scale_ui = 800.0 / wall_width

# ---- Synchroniseer drag-posities voor nieuwe runs ----
try:
    from streamlit_js_eval import streamlit_js_eval
    positions = streamlit_js_eval(
        js_expressions=[
            "(function(){"
            " let arr=[];"
    ...
    if positions:
        arr = json.loads(positions)
        for pos in arr:
            for p in st.session_state.panels:
                if p["id"] == pos["id"]:
                    p["x"], p["y"], p["rotation"] = pos["x"], pos["y"], pos["rotation"]
except ImportError:
    st.warning("Voeg 'streamlit-js-eval' toe aan requirements.txt voor drag-persistency.")
